...
- **Q3 — Widget data source.** Herd widget: shell out to `herdr agent list` (JSON) per render tick, or hold the socket? Polling interval + cache strategy to stay flicker-free? *Decide in formulation; CLI-per-tick with ~2s cache is the simple start.*
