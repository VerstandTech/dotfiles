...
- `text` — soft ink (`#e0e0e0` dark / `#2a2a2a` light)
...
### 7.3 Working row (component) — `tui-chrome`
Target: `◐ Working... ████████████████`
- Hide built-in loader while agent runs; above-editor widget with moon + label + progress bar
- Pure renderer: `lib/tui-chrome/working-row.ts`. ops-hud owns status chips only.
